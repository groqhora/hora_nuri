...
class AIEngine:
    def __init__(self):
        self.groq_keys = []
        for key_name in ["GROQ_API_KEY", "GROQ_API_KEY_1", "GROQ_API_KEY_2", "GROQ_API_KEY_3"]:
            val = os.getenv(key_name)
            if val:
                clean = val.strip().strip('"').strip("'")
                if clean and clean not in self.groq_keys:
                    self.groq_keys.append(clean)
                    if len(self.groq_keys) >= 3:
                        break

        raw_gemini = os.getenv("GEMINI_API_KEY") or os.getenv("GEMINI_KEY")
        self.gemini_key = raw_gemini.strip().strip('"').strip("'") if raw_gemini else None
        ...
        for attempt in range(max_retries):
            try:
                ...
                if engine.startswith("groq") and self.groq_client:
                    response = await asyncio.to_thread(self._groq_response, prompt)
                    return response
                elif engine == "gemini" and self.gemini_client:
                    response = await asyncio.to_thread(self._gemini_response, prompt)
                    return response
                else:
